...
function renderAdmin(){
  app.innerHTML = `<div class="app"><div class="top"><div><b style="color:#00d4a0">ADMIN CRM</b><br><button id="logout" class="btn red">CERRAR SESIÓN</button></div><b>${currentModule}</b></div><div class="main"><div class="side" id="moduleList"></div><div class="content" style="overflow:auto"><div class="panel"><h2 style="color:#63b3ed">${currentModule}</h2><div class="grid"><div class="panel">Crear / editar registros</div><div class="panel">Estados y validaciones</div><div class="panel">Historial de actividad</div><div class="panel">Auditoría futura</div></div></div><div id="extra"></div></div></div></div>`;
...
function renderTrade(){
  ...
  assets.forEach(a => { const div = document.createElement('div'); div.className = 'item ' + (a[0] === selectedAsset ? 'active' : ''); div.innerHTML = a[0] + '<br><small>' + a[1] + '</small>'; div.addEventListener('click', () => { selectedAsset = a[0]; render(); }); list.appendChild(div); });
...
function renderMoneyForm(type){
  ...
  document.getElementById('confirmMoney').addEventListener('click', () => { const amount = Number(document.getElementById('moneyAmount').value); if (!amount || amount <= 0) return; money.unshift({type, amount, status: type === 'Depósito' ? 'Pendiente de autorización' : 'Solicitud pendiente', time: now()}); if (type === 'Retiro') balance -= amount; activeTab = 'money'; setPage('trade'); });
}

function executeOrder(){
  const vol = Number(document.getElementById('lots').value); const err = document.getElementById('orderError');
  ...
  positions.unshift({id: Date.now(), symbol:selectedAsset, side:selectedSide, lots:vol, pnl:0, time:now()}); activeTab='open'; render();
}
...
function closePosition(id){ const idx = positions.findIndex(p=>p.id===id); if (idx<0) return; const p = positions.splice(idx,1)[0]; closed.unshift({...p, time:now()}); balance += p.pnl; activeTab='closed'; render(); }
...
function drawChart(){
  ...
  const rect = c.parentElement.getBoundingClientRect(); const dpr = window.devicePixelRatio || 1;
  const cssW = Math.max(500, Math.floor(rect.width)); const cssH = Math.max(260, Math.floor(rect.height));
  c.width = Math.floor(cssW * dpr); c.height = Math.floor(cssH * dpr); c.style.width = cssW + 'px'; c.style.height = cssH + 'px';
  const ctx=c.getContext('2d'); ctx.setTransform(dpr,0,0,dpr,0,0);
  const W=cssW,H=cssH; ctx.clearRect(0,0,W,H); ctx.fillStyle='#060d1a'; ctx.fillRect(0,0,W,H);
  const left=58,right=76,top=16,bottom=34,cw=W-left-right,ch=H-top-bottom; let base=getPrice(),data=[],p=base;
  ...
  let min=Math.min(...data.map(d=>d.low)),max=Math.max(...data.map(d=>d.high)),range=max-min||1; const y=v=>top+ch-((v-min)/range)*ch;
  for(let i=0;i<=6;i++){let yy=top+i*ch/6;ctx.strokeStyle='#111d30';ctx.beginPath();ctx.moveTo(left,yy);ctx.lineTo(W-right,yy);ctx.stroke();let label=max-i*range/6;ctx.fillStyle='#5a6e8a';ctx.font='11px Courier New';ctx.textAlign='right';ctx.fillText(label.toFixed(base>100?2:5),left-6,yy+4)}
  ...
  ctx.strokeStyle='#f6ad55';ctx.beginPath();for(let i=19;i<data.length;i++){let avg=data.slice(i-19,i+1).reduce((s,d)=>s+d.close,0)/20;let x=left+i*step+step/2;i===19?ctx.moveTo(x,y(avg)):ctx.lineTo(x,y(avg))}ctx.stroke();
  ctx.setLineDash([4,4]);ctx.strokeStyle='#63b3ed';let ly=y(data[data.length-1].close);ctx.beginPath();ctx.moveTo(left,ly);ctx.lineTo(W-right,ly);ctx.stroke();ctx.setLineDash([]);
}
